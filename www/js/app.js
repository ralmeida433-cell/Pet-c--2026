class HotelPetApp {
    ...
    async init() {
        try {
            ...
            // Garantir tempo mínimo para a animação premium (4 segundos)
            setTimeout(async () => {
                this.hideLoading();
    ...
    initializeManagers() {
        ...
        if (typeof SettingsManager !== 'undefined') window.settingsManager = new SettingsManager();
        if (typeof PostsManager !== 'undefined') window.postsManager = new PostsManager();
    ...
    bindGlobalEvents() {
        ...
        // CORREÇÃO: Forçar abertura do calendário ao clicar no input date
        document.addEventListener('click', (e) => {
            if (e.target.tagName === 'INPUT' && (e.target.type === 'date' || e.target.type === 'month')) {
                try {
                    e.target.showPicker();
                } catch (err) {
                    console.log('showPicker não suportado ou bloqueado', err);
                }
            }
        });
    }
    ...
    async navigateToSection(sectionName, addToHistory = true) {
        if (!this.isInitialized || this.currentSection === sectionName) return;
        ...
        if (oldSection) {
            ...
            setTimeout(async () => {
                ...
                if (newSection) {
                    ...
                    // Pequeno delay para forçar reflow e trigger da transição de entrada
    ...
    hideLoading() {
        ...
        }, 1200); // Batches with CSS transition duration
    }
