class InventoryManager {
    ...
    async initializeAfterDOM() {
        try {
            ...
            console.warn('Inventory Manager init deferred or failed:', error);
        }
    }
    ...
    handleImageFile(file) {
        ...
        reader.onload = async (e) => {
            ...
            if (window.storageService) {
                try {
                    imageSrc = await window.storageService.saveImage(imageSrc);
                } catch (err) {
                    console.error('Erro ao salvar imagem de produto:', err);
                }
            }
    ...
    async loadInventory() {
        try {
            ...
            if (window.db) {
                ...
                this.sales = await window.db.getSales();
            }
    ...
    async saveProduct() {
        ...
        await this.processSaveProduct(imageUrl);
    }

    async processSaveProduct(imageUrl) {
        const productData = {
    ...
    async updateProduct(productId, productData) {
        ...
        await window.db.updateProduct(productId, updatedProduct);
    ...
    async updateStock(productId) {
        ...
        if (newStock !== null && !isNaN(newStock)) {
            ...
            await this.updateProduct(productId, { currentStock: stockVal });
