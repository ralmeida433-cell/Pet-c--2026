class Database {
    ...
    async getAnimals(search = '', kennelType = '') {
        ...
        const params = [];
        if (search) { query += ` AND (name LIKE ? OR tutor_name LIKE ?)`; params.push(`%${search}%`, `%${search}%`); }
        if (kennelType) { query += ` AND species = ?`; params.push(kennelType); }
        query += ` ORDER BY name`;
    ...
    async insertSampleData() {
        ...
        sampleAnimals.forEach(animal => { this.db.run(`INSERT INTO animals (name, species, tutor_name, tutor_phone, photo_url) VALUES (?, ?, ?, ?, ?)`, animal); });
        await this.saveData();
    }
    
    async getReservations(search = '', status = '', month = '') { let query = `SELECT r.*, a.name as animal_name, a.species as animal_species, a.tutor_name FROM reservations r JOIN animals a ON r.animal_id = a.id WHERE 1=1`; const params = []; if (search) { query += ` AND (a.name LIKE ? OR a.tutor_name LIKE ?)`; params.push(`%${search}%`, `%${search}%`); } if (status) { query += ` AND r.status = ?`; params.push(status); } if (month) { query += ` AND strftime('%Y-%m', r.checkin_date) = ?`; params.push(month); } query += ` ORDER BY r.checkin_date DESC`; return this.executeQuery(query, params); }
    ...
    async getDashboardStats() { const totalAnimals = this.executeQuerySingle('SELECT COUNT(*) as count FROM animals'); const activeReservations = this.executeQuerySingle(`SELECT COUNT(*) as count FROM reservations WHERE status = 'ATIVA'`); const monthlyRevenue = this.executeQuerySingle(`SELECT COALESCE(SUM(total_value), 0) as revenue FROM reservations WHERE strftime('%Y-%m', checkin_date) = strftime('%Y-%m', 'now') AND status IN ('ATIVA', 'FINALIZADA')`); return { totalAnimals: totalAnimals?.count || 0, activeReservations: activeReservations?.count || 0, monthlyRevenue: monthlyRevenue?.revenue || 0, occupancyRate: Math.round((activeReservations.count / 15) * 100) }; }
    ...
    async getProducts() { const products = this.executeQuery('SELECT * FROM products ORDER BY name'); return products.map(p => ({ ...p, animals: JSON.parse(p.animals || '[]'), costPrice: p.cost_price, sale_price: p.sale_price, current_stock: p.current_stock, min_stock: p.min_stock, expiryDate: p.expiry_date, imageUrl: p.image_url })); }
    async addProduct(p) { this.db.run(`INSERT INTO products (id, name, category, brand, animals, cost_price, sale_price, current_stock, min_stock, weight, expiry_date, supplier, description, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [p.id, p.name, p.category, p.brand, JSON.stringify(p.animals), p.costPrice, p.sale_price, p.current_stock, p.min_stock, p.weight, p.expiryDate, p.supplier, p.description, p.imageUrl]); await this.saveData(); }
    async updateProduct(id, p) { this.db.run(`UPDATE products SET name = ?, category = ?, brand = ?, animals = ?, cost_price = ?, sale_price = ?, current_stock = ?, min_stock = ?, weight = ?, expiry_date = ?, supplier = ?, description = ?, image_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [p.name, p.category, p.brand, JSON.stringify(p.animals), p.costPrice, p.sale_price, p.current_stock, p.min_stock, p.weight, p.expiryDate, p.supplier, p.description, p.imageUrl, id]); await this.saveData(); }
    async deleteProduct(id) { this.db.run('DELETE FROM products WHERE id = ?', [id]); await this.saveData(); }
    async getSales() { const sales = this.executeQuery('SELECT * FROM sales ORDER BY date DESC'); return sales.map(s => ({ ...s, productId: s.product_id, productName: s.product_name, unitPrice: s.unit_price, total_price: s.total_price, cost_price: s.cost_price, total_cost: s.total_cost })); }
    async addSale(s) { this.db.run(`INSERT INTO sales (id, product_id, product_name, quantity, unit_price, total_price, cost_price, total_cost, profit, date, customer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [s.id, s.product_id, s.product_name, s.quantity, s.unit_price, s.total_price, s.cost_price, s.total_cost, s.profit, s.date, s.customer]); await this.saveData(); }
}
