class HotelPetApp {
    ...
    async init() {
        try {
            ...
            // Inicializar Capacitor (se disponível)
            ...
            // Ocultar Splash Screen Nativa após carregamento
    ...
    async navigateToSection(sectionName, addToHistory = true) {
        ...
        try {
            // Se já estamos nesta seção, não faz nada (exceto se for histórico)
            if (this.currentSection === sectionName && !this.isNavigatingHistory) return;

            // Adicionar ao histórico
            if (addToHistory) {
                // Se estamos no meio do histórico e navegamos para nova seção,
                // removemos o "futuro"
            ...
            // Atualizar UI dos botões de navegação
            ...
            // Atualizar título no header mobile
    ...
    async loadSectionData(sectionName) {
        try {
            switch (sectionName) {
                case 'overview':
                    // Registra overview no histórico inicial se estiver vazio
    ...
    goBack() {
        if (this.historyIndex > 0) {
            ...
            this.navigateToSection(this.history[this.historyIndex], false);
            this.isNavigatingHistory = false;
        }
    }
