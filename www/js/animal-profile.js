class AnimalProfileManager {
        ...
        container.innerHTML = `
            <div class="profile-header-card">
                <div class="profile-photo-area">
                    <img src="${animal.photo_url || ''}" alt="${animal.name}" class="profile-photo" onerror="this.style.display='none'">
                </div>
    ...
    renderTimeline(history) {
        if (!history || history.length === 0) {
            return '<p class="text-center text-secondary" style="padding: 2rem;">Nenhum registro.</p>';
        }
    ...
    async saveHistoryEntry() {
        ...
        try {
            ...
            window.hotelPetApp.showNotification('Serviço registrado!', 'success');
    ...
    async deleteHistoryEntry(id) {
        if (confirm('Excluir?')) {
            try {
                ...
                window.hotelPetApp.showNotification('Removido.', 'success');
                this.loadHistory(this.currentAnimal.id);
            } catch (e) {
                console.error(e);
            }
